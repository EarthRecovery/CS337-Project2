...
class LLM_Model:
    ...
    def generate_content(self, **kwargs):
        ...
        if hasattr(resp, "text"):
            return resp.text  # genai 0.1.x convenience attr
        # Fallback to first candidate text if needed
        try:
...
class Parser:
    ...
    def __init__(self, data_url, LLM_model=None):
    ...
    def LLM_parser(self):
        ...
        raw_website = resp.text
        with open("prompts.json", "r") as f:
            demo_structed_parsed_data = json.load(f)["demo_parsed_data"]
        prompt = f"""
        ...
        match = re.search(r"```json\s*(.*?)```", raw_text, re.S)
        if match:
...
if __name__ == "__main__":
    llm_model = LLM_Model()  # Placeholder for LLM_Model instance
    parser = Parser("https://www.allrecipes.com/recipe/218091/classic-and-simple-meat-lasagna/", LLM_model=llm_model.model)
    parsed = parser.parse(use_LLM=True)
